...
const GroupMemberModal = ({ setIsGroupMemberModalOpen, filterGroup }: GroupMemberModalProps) => {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/50 px-3">
            <div ref={wraperRef} className="w-full max-w-xl rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 shadow-2xl transition-colors duration-200">
                <div className="flex items-start justify-between border-b border-slate-200 dark:border-slate-700 px-6 py-5">
                    <div className="flex items-start gap-3">
                        ...
                        <div>
                            <h2 className="text-lg font-semibold text-slate-900 dark:text-slate-100">Group members</h2>
                            <p className="text-xs text-slate-500 dark:text-slate-400">People currently in this group.</p>
                        </div>
                    </div>
                    <button
                        type="button"
                ...
                <div className="px-6 py-5">
                    ...
                    <div className="max-h-72 space-y-2 overflow-y-auto pr-1 customScrollbar">
                        {filteredMembers.length === 0 ? (
                            <div className="rounded-lg border border-dashed border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-800/50 px-3 py-4 text-center text-xs text-slate-500 dark:text-slate-400 transition-colors duration-200">
                                No members found for this group.
                            </div>
                        ) : (
                            filteredMembers.map((member) => (
                                <div key={member.id} className="flex items-center justify-between rounded-lg border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-800 px-3 py-2 transition-colors duration-200">
                                    <div className="flex min-w-0 items-center gap-3">
                                        ...
                                        <div className="min-w-0">
                                            <p className="truncate text-sm font-medium text-slate-900 dark:text-slate-100">{member.username}</p>
                                            {member.email && <p className="truncate text-xs text-slate-500 dark:text-slate-400">{member.email}</p>}
                                        </div>
                                    </div>
                                    <span className="rounded-full bg-slate-100 dark:bg-slate-700 px-2 py-0.5 text-[11px] text-slate-600 dark:text-slate-300 transition-colors duration-200">{member.email === user?.email ? "You" : "Member"}</span>
                                </div>
                            ))
                        )}
                    </div>
