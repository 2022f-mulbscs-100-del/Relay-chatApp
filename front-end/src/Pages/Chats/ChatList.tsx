...
const ChatList = ({
    ...
    return (
        <div
            className={`group w-full flex items-center gap-3 px-3 py-3 rounded-xl border ${activeChat ? "border-slate-300 dark:border-slate-600 shadow-sm" : "border-slate-200 dark:border-slate-700"} bg-white dark:bg-slate-800 hover:border-slate-300 dark:hover:border-slate-600 hover:shadow-sm cursor-pointer transition`}
            onClick={() => {
        ...
        >
            ...
            <div className="flex flex-col w-full min-w-0">
                <div className="flex items-center justify-between gap-2">
                    ...
                    <span className="text-[11px] text-slate-400 dark:text-slate-500">{receivedMessages && receivedMessages.length > 0 ? new Date(receivedMessages[receivedMessages.length - 1].createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : ""}</span>
                </div>
                <div className="flex items-center justify-between gap-2">
                    <p className="text-xs text-slate-500 dark:text-slate-400 truncate">{lastMessage ? lastMessage.content : "No messages yet"}</p>
                    <div className="flex items-center gap-1.5 shrink-0">
                        {mode === "group" && isMuted && (
                            <span className="inline-flex items-center justify-center text-slate-400 dark:text-slate-500" title="Muted group">
                        ...
                        {mode === "private" && getUnreadCount() > 0 &&
                            <span className="min-w-[18px] h-[18px] rounded-full bg-slate-900 dark:bg-white text-white dark:text-slate-900 text-[10px] flex items-center justify-center">
                                {getUnreadCount()}
                            </span>
                        }
